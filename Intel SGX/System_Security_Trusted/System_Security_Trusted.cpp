...
typedef struct _sgx_ec256_public_hex_t
{
	char gx[65];
	char gy[65];
} sgx_ec256_public_hex_t;

typedef struct _sgx_ec256_signature_hex_t
{
	char x[64];
	char y[64];
} sgx_ec256_signature_hex_t;
...
int esv_sign(const char* message,  void* signature, size_t sig_len)
{
	...
	_sgx_ec256_public_hex_t clave_publica;
	_sgx_ec256_public_hex_t firma;
	//char claveX[64];
	//char claveY[64];
	...
	for (int i = 0; i < 32; i++)
	{
		uint8_t auxX = p_public.gx[32 - i];
		uint8_t auxY = p_public.gy[32 - i];
		std::snprintf(&clave_publica.gx[i * 2], 65, "%02X", (uint8_t)p_public.gx[32 - 1 - i]);
		std::snprintf(&clave_publica.gy[i * 2], 65, "%02X", (uint8_t)p_public.gy[32 - 1 - i]);
	}
	
	for (int i = 0; i < 8; i++)
	{
		uint32_t sigX = (sig)->x[8 - 1 - i];
		uint32_t sigY = (sig)->y[8 - 1 - i];
		std::snprintf(&firma.gx[i * 8], 65, "%08X", sigX);
		std::snprintf(&firma.gy[i * 8], 65, "%08X", sigY);
	}

	if(ret == SGX_SUCCESS)
	{
		esv_write_data("sign2", (unsigned char*)&firma, sizeof(_sgx_ec256_public_hex_t));
		esv_write_data("public_key2", (unsigned char*)&clave_publica, sizeof(_sgx_ec256_public_hex_t));
	}
